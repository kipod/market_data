...
.md.genBboZ18: delete ttime from update time:ttime, "i"$ask, "i"$bid, "i"$asize%100, "i"$bsize%100 from `ttime xasc .md.genBboZ18

.md.bboMatch:{[day;xchng;sid]
    //symbols:$[xchng="Z"; .BATS.symbols; xchng="Q"; .NASDAQ.symbols; xchng="N"; .NYSE.symbols; xchng in "PT"; .ARCA.symbols];
    //symids:.md.getSymID[`date$day] each symbols;
    symids:(72);
    //exec distinct symbolid from .md.genBboZ18 where date=day, ex=xchng;
    ...
    genbbo:`time xasc update "P"$string time from select date,time, symbolid, bidvol:bsize, bidprice:bid, askvol:asize, askprice:ask from .md.genBboZ18 where date=day, ex=xchng, symbolid in symids;
    w:"P"$string (.md.minDTime;.md.maxDTime)+\:genbbo[`time];
    update td:{d:x-y;r:min d where d>0;$[0Wj=`long$r;0Nn;r]}'[ttime;time] from
        wj[w;`symbolid`bidvol`bidprice`askvol`askprice`time;genbbo;(bbo;(::;`ttime))]}


.res.genBboZ18:.md.bboMatch [7230;"P";73]

count select from .md.orders where date=7230, symbolid=72,ex="P"
